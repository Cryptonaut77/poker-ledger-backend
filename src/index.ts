...
const envPath = path.resolve(import.meta.dir, `../${envFile}`);
console.log(`[dotenv@${process.env.NODE_ENV}] Loading from: ${envPath}`);
// Don't override existing env vars (e.g. DATABASE_URL set by scripts/env.sh in production)
const result = config({ path: envPath, override: false });
console.log(`[dotenv@${process.env.NODE_ENV}] injecting env (${Object.keys(result.parsed || {}).length}) from ${envFile} -- tip: ensure .env.production has correct OPENAI_API_KEY`);
