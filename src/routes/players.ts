...
playersRouter.put("/transaction/:id/mark-paid", async (c) => {
  ...
  // Verify the transaction belongs to a session owned by this user or where user is a member
  const existingTransaction = await db.playerTransaction.findUnique({
    where: { id },
    include: {
      gameSession: {
  ...
  const isOwner = existingTransaction?.gameSession.userId === user.id;
  const isMember = existingTransaction?.gameSession.members.some((m) => m.userId === user.id);

  if (!existingTransaction || (!isOwner && !isMember)) {
    return c.json({ error: "Transaction not found" }, 404);
  }
