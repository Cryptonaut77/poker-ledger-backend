...
// Apply auth middleware to all share routes EXCEPT the check endpoint
shareRouter.get("/check/:code", async (c) => {
  ...
  const gameSession = await db.gameSession.findUnique({
    ...
    select: { id: true, shareCodeExpiresAt: true, isActive: true },
  });
  if (!gameSession) {
    return c.json({ valid: false, reason: "Code not found in database" });
  }
  if (gameSession.shareCodeExpiresAt && new Date() > gameSession.shareCodeExpiresAt) {
    return c.json({ valid: false, reason: "Code expired" });
  }
  return c.json({ valid: true, active: gameSession.isActive });
});
...
shareRouter.post("/join", zValidator("json", joinSchema), async (c) => {
  ...
  try {
    ...
    // Deactivate the joining user's own active game (if any) so that
    // GET /api/game/active returns the shared game instead.
    // Only deactivate games that have NO transactions/data (auto-created empty games).
    const ownActiveGame = await db.gameSession.findFirst({
      where: { userId: user.id, isActive: true },
      include: {
        _count: {
          select: {
    ...
    if (ownActiveGame) {
      const hasData =
        ownActiveGame._count.playerTransactions > 0 ||
        ownActiveGame._count.dealerDowns > 0 ||
        ownActiveGame._count.expenses > 0;

      if (!hasData) {
        // Empty auto-created game - safe to deactivate
        await db.gameSession.update({
          where: { id: ownActiveGame.id },
          data: { isActive: false },
        });
        console.log(`🔗 [Share] Deactivated empty game ${ownActiveGame.id} for user ${user.email}`);
      } else {
        console.log(`🔗 [Share] User ${user.email} has an active game with data - keeping it but membership will be secondary`);
      }
    }
